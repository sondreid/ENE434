...
pv_df %<>% mutate(cum_cap =cumsum(nameplate))  %>%  filter(cost_per_kw !=0)
...
pv_df_log <- df_pre_2012  %>%  bind_rows(df_post_2012)

loess_fit <-   pv_df %>% loess(log2(cost_per_kw)~log2(cum_cap), span=.2, data = ., control=loess.control(surface="direct"))
```
...
ggplot(pv_df_log, aes(log2_cum_cap, log2_cost_per_kw)) + 
geom_point() +
...
pv_df_month <- pv_df_log  %>% group_by(monthdate)  %>% summarise(log2_cost_per_kw = sum(log2_cost_per_kw))  %>% as_tibble()
...
updated_df <- read_csv("WorkingDataSet.csv")
